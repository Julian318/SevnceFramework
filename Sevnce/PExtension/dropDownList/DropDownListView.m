...
- (id)initWithFrame:(CGRect)frame dataSource:(id)datasource delegate:(id) delegate
{
    ...
    if (self) {
        ...
        for (int i = 0; i <sectionNum; i++) {
            ...
            sectionBtn.layer.cornerRadius=4;
            sectionBtn.layer.borderColor=[[UIColor lightGrayColor] CGColor];
...
{
    if (!self.mTableView) {
        self.mTableBaseView = [[UIView alloc] initWithFrame:CGRectMake(0, self.frame.origin.y + self.frame.size.height , screenwidth, self.mSuperView.frame.size.height - self.frame.origin.y - self.frame.size.height)];
        ...
        self.mTableView = [[UITableView alloc] initWithFrame:CGRectMake(self.frame.origin.x, self.frame.origin.y + self.frame.size.height, self.frame.size.width, 80) style:UITableViewStylePlain];
