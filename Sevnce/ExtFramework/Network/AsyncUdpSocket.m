...
- (void)runLoopAddSource:(CFRunLoopSourceRef)source
{
	...
	for(i = 0; i < count; i++)
	{
		CFStringRef runLoopMode = (CFStringRef)CFBridgingRetain([theRunLoopModes objectAtIndex:i]);
...
- (void)maybeDequeueSend
{
	...
	if(theCurrentSend == nil)
	{
		if([theSendQueue count] > 0)
		{
            ...
            //NSLog(@"SendQueue count is %d,ReceiveQueue count is %d",[theSendQueue count],[theReceiveQueue count]);
...
{
	AsyncUdpSocket *theSocket = (AsyncUdpSocket *)CFBridgingRelease(pInfo);
	[theSocket doCFSocketCallback:type forSocket:sref withAddress:(NSData *)CFBridgingRelease(address) withData:pData];
}
