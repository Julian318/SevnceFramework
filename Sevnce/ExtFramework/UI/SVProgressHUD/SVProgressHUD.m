...
- (void)showWithStatus:(NSString*)string maskType:(SVProgressHUDMaskType)hudMaskType networkIndicator:(BOOL)show {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if([self.spinnerView isKindOfClass:[UIImageView class]]){
            [((UIImageView*)self.spinnerView) startAnimating];
        }else if([self.spinnerView isKindOfClass:[UIActivityIndicatorView class]]){
...
- (void)dismissWithStatus:(NSString *)string error:(BOOL)error afterDelay:(NSTimeInterval)seconds {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        [self setStatus:string];
        if([self.spinnerView isKindOfClass:[UIImageView class]]){
            [((UIImageView*)self.spinnerView) stopAnimating];
        }else if([self.spinnerView isKindOfClass:[UIActivityIndicatorView class]]){
            [((UIActivityIndicatorView*)self.spinnerView) stopAnimating];
        }
...
- (UIView *)spinnerView {
    if (spinnerView == nil) {
        UIActivityIndicatorView* temp = [[UIActivityIndicatorView alloc] initWithActivityIndicatorStyle:UIActivityIndicatorViewStyleWhiteLarge];
		temp.hidesWhenStopped = YES;
		temp.bounds = CGRectMake(0, 0, 37, 37);
        spinnerView=temp;
    }
