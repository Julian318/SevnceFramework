...
-(void)generateRadialMenu{
	...
	if([self.dataSource respondsToSelector:@selector(viewForCenterOfRadialMenu:)]){
		...
		if(_centerRadialView){
            _centerRadialView.center = CGPointMake(self.radialMenuView.frame.size.width*0.5, self.radialMenuView.frame.size.height*0.5);
...
-(void)showMenuWithCompletion:(void (^)())completion{
	...
	[UIView animateWithDuration:0.25 animations:^{
		...
		[UIView animateWithDuration:0.25 animations:^{
			if(_centerRadialView){
				CGFloat centerViewRadius = _centerRadialView.bounds.size.width*0.5;
